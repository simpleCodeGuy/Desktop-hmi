...
class ModbusBlockId {
  static const elementTypeVsNumber = {
    ElementType.coil: 0,
    ElementType.discreteInput: 1,
    ElementType.inputRegister: 3,
    ElementType.holdingRegister: 4,
  };
  ...
  bool IS_EQUAL_TO(ModbusBlockId OTHER) {
    return ip == OTHER.ip &&
        port == OTHER.port &&
        readWrite == OTHER.readWrite &&
        elementType == OTHER.readWrite &&
        elementNumber == OTHER.elementNumber;
  }

  String asString() {
    String readString = readWrite == ReadWrite.read ? '1' : '0';
    return '$ip,$port,$readString,${elementTypeVsNumber[elementType]},$elementNumber';
  }
...
typedef RequestToModbus = Request;
typedef ResponseFromModbus = Response;
...
class Response {
  ...
  Response COPY() {
    return Response(
      modbusBlockId: modbusBlockId.COPY(),
      isSuccess: isSuccess,
      readData: readData,
      transactionId: transactionId,
    );
...
class ModbusMasterForWorker {
  ...
  /// This method is started as a child isolate by 'start' method of Modbus
  /// Master object.
  ///
  /// #### Features of this method
  /// - sends sendport of worker isolate's new receivePort to enable
  ///   bidirectional communication between parent isolate and worker isolate
  ///   - parent isolate has its own receivePort and worker isolate's
  ///     sendPort
  ///   - worker isolate has its own receivePort and parent isolate's
  ///     sendPort
  /// - sets up response stream emitting modbusResponseData so that
  ///   modbusResponseData is sent to parentIsolate
  /// - sets up receivePort of worker isolate such that modbusRequestData
  ///   received from parent isolate is processed and its corresponding
  ///   modbus ADU is sent to slave
